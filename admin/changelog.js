...
async function generate() {
    ...
    changes.forEach((change) => {
        ...
        let link = body.match(/(\((.*#.*)\))/)
        let commit = `[${ change.commit.substring(0, 7) }](https://github.com/astra-x/crypu.js/commit/${ change.commit })`;
        if (link) {
            body = body.replace(/ *(\(.*#.*)\) */, '');
            link = link[2].replace(/#([0-9]+)/g, (all, issue) => {
                return `[#${ issue }](https://github.com/astra-x/crypu.js/issues/${ issue })`;
            }) + '; ' + commit;
        } else {
            link = commit;
        }
